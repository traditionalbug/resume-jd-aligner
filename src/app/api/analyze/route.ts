...
import { MODELS, openai, anthropic, gemini, factSystem, extractJson } from "@/lib/llms"; // removed criticSystem
import { z } from "zod";
import {
  FactsJSON,
  DiffJSON,
...
function computeDiff(facts: TFactsJSON, jd: string) {
  ...
  const raw = {
    supported_keywords: supported.slice(0, 200),
    missing_keywords: missing.slice(0, 200),
  };
  return DiffJSON.parse(raw);
}
...
// --- Stage 4: Validator (drop unsupported bullets) ---
function validateProvenance(editorJson: TEditorJSON, facts: TFactsJSON): TEditorJSON {
  ...
  const aligned = (editorJson.aligned_resume || []).filter(
    (b) => Array.isArray(b.source_ids) && b.source_ids.length > 0 && b.source_ids.every((id) => factIds.has(id))
  );
  const safe: TEditorJSON = {
    ...editorJson,
    aligned_resume: aligned,
  };
...
export async function POST(req: Request) {
  try {
    ...
    // 1) Extract facts in parallel
    const results = await Promise.allSettled<TFactsJSON>([
    ...
    const bags: TFactsJSON[] = results
      ...
      .filter((x): x is TFactsJSON => x !== null);

    if (bags.length === 0) {
      // Fallback: minimally treat resume tokens as skill facts
      const tokens = tokenize(resume).slice(0, 100);
      const facts: TFact[] = tokens.map((t, idx) => ({
        id: `f${idx + 1}`,
        type: "skill",
        text: t,
      }));
